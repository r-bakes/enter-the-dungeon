...
export default function TaskInfo({
    ...
    const {setWorkingSkill, setWorkingTask, progress, workingTask, character} = useEngineContext();
            ...
            <div className="flex flex-col w-full h-full ">
                ...
                <CardContent>
                    ...
                    <div className="flex w-full py-2">
                            ...
                            {taskRequires.map((Item) => (
                                <div className="flex flex-col text-center w-20 border rounded-sm items-center p-2 mr-2" key={Item.item.id}>
                                    ...
                                    <div className="flex flex-col">
                                        <Label className="text-xs text-muted-foreground">{Item.quantity}</Label>
                                        <div className="flex items-center justify-end w-full space-x-1">
                                            <Label className="text-xs text-muted-foreground">({Item.item.id in character.inventory.items ? character.inventory.items[Item.item.id] : 0})</Label>
                        ...
                        <div className="w-3/4 text-right">
                            <Label className="text-s text-muted-foreground pointer-events-none">requires</Label>
                        </div> 
                    </div> : <></>
                    }
                </CardContent>
                <CardFooter className="flex w-full grow items-end">
                    <Button className="rounded-l-md rounded-r-none w-1/2 text-center" disabled={!requirementsMet} onClick={() => {setWorkingSkill(skill); setWorkingTask(task)}}>
