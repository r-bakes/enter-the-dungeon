                    ...
                    <ScrollArea className="flex h-full w-full overflow-hidden pr-4"> 
                        <div className="flex flex-col space-y-4 w-full h-full">
                            {Object.entries(skill.tasks).map(([category, tasks]) => (
                                <div className="flex flex-col space-y-2 overflow-hidden w-full" key={category}>
                                    <Label className="text-xl text-muted-foreground font-extralight">{category.charAt(0).toUpperCase() + category.slice(1)}</Label>
                                    <TaskContainer skillLevel={character.skills.data[skill.id].level} tasks={tasks} setTask={setTask}></TaskContainer>
                                </div>
                                ))}
                        </div>
